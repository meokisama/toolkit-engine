...
export function registerDeviceConfigHandlers(ipcMain, rcu) {
  ...
  ipcMain.handle("rcu:syncClock", async (event, { unitIp, canId, clockData }) => {
    try {
  ...
  ipcMain.handle("rcu:changeIpAddress", async (event, { unitIp, canId, data }) => {
    try {
  ...
  ipcMain.handle("rcu:changeIpAddressBroadcast", async (event, { unitIp, canId, newIpBytes, oldIpBytes }) => {
    try {
      return await rcu.changeIpAddressBroadcast(unitIp, canId, newIpBytes, oldIpBytes);
    } catch (error) {
  ...
  ipcMain.handle("rcu:changeCanId", async (event, { unitIp, canId, newLastPart }) => {
    try {
  ...
  ipcMain.handle("rcu:setHardwareConfig", async (event, { unitIp, canId, configByte }) => {
    try {
  ...
  ipcMain.handle("rcu:setRS485CH1Config", async (event, { unitIp, canId, config }) => {
    try {
  ...
  ipcMain.handle("rcu:setRS485CH2Config", async (event, { unitIp, canId, config }) => {
    try {
