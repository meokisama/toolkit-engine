...
import { Network, Scan, Wifi, CircleCheck, Loader2, CheckSquare, Square } from "lucide-react";
            ...
            {networkUnits.length > 0 && (
              <>
                <Button
                  ...
                  disabled={disabled || selectedUnitIds.length === networkUnits.length || (maxSelection && selectedUnitIds.length >= maxSelection)}
                  size="sm"
                  variant="outline"
                >
                  <CheckSquare className="h-4 w-4" />
                  {maxSelection ? `Select ${Math.min(maxSelection, networkUnits.length)}` : "Select All"}
                </Button>
                <Button onClick={handleSelectNone} disabled={disabled || selectedUnitIds.length === 0} size="sm" variant="outline">
                  <Square className="h-4 w-4" />
                  Select None
                </Button>
              </>
            )}
            <Button onClick={handleScanNetwork} disabled={scanLoading || disabled} size="sm" variant="outline">
              {scanLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Scan className="h-4 w-4" />}
        ...
        <ScrollArea className={height}>
          {networkUnits.length === 0 ? (
            <div className="text-center text-muted-foreground py-8">
              ...
              <p className="text-sm">Click "Scan Network" to discover units on your network.</p>
            </div>
          ) : (
              ...
              {networkUnits.map((unit) => (
                ...
                >
                  ...
                  <div className="space-y-1">
                    <span className="font-medium tracking-tight text-sm">{unit.type || unit.unit_type || "Unknown Unit"}</span>
                    <p className="text-xs text-muted-foreground">IP: {unit.ip_address}</p>
                    {unit.id_can && <p className="text-xs text-muted-foreground">CAN ID: {unit.id_can}</p>}
                  </div>
