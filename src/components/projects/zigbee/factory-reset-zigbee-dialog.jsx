...
export function FactoryResetZigbeeDialog({ open, onOpenChange, devices, projectId, onSuccess }) {
  ...
  const handleFactoryReset = async () => {
    ...
    try {
      ...
      const result = await window.electronAPI.zigbeeController.factoryResetZigbee({
        unitIp,
        canId,
      });
      ...
      if (result.success) {
        ...
        toast.success(`Factory reset successful for unit ${unitIp}. All paired devices have been removed from both the coordinator and database.`);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>
            This will remove all paired Zigbee devices from the selected unit's coordinator and reset the network. This action cannot be undone.
          </DialogDescription>
        </DialogHeader>

        <div className="space-y-4 py-4">
          {units.length === 0 ? (
            <div className="text-center text-muted-foreground py-4">No units found with Zigbee devices</div>
          ) : (
            <div className="space-y-2">
              ...
              <Select value={selectedUnit} onValueChange={setSelectedUnit}>
                ...
                <SelectContent>
                  {units.map((unit) => (
                    <SelectItem key={`${unit.unitIp}|${unit.canId}`} value={`${unit.unitIp}|${unit.canId}`}>
                      {unit.unitIp} (CAN ID: {unit.canId}) - {unit.deviceCount} device(s)
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
        ...
        <DialogFooter>
          ...
          <Button variant="destructive" onClick={handleFactoryReset} disabled={loading || units.length === 0 || !selectedUnit}>
