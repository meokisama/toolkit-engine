                                      ...
                                      {!hasUnmappedAddress(config.slave_cfg[slaveIndex]?.slave_group || 0) ? (
                                        <Combobox
                                          ...
                                          value={getMappedComboboxValue(
                                            config.slave_cfg[slaveIndex]?.slave_group || 0
                                          )}
                                          onValueChange={(value) =>
                                            handleSlaveConfigChange(
                                              configIndex,
                                        ...
                                        <div className="flex-1 px-3 py-2 border rounded-md bg-muted text-muted-foreground text-sm">
                                          Address {config.slave_cfg[slaveIndex]?.slave_group || 0} (Not in database)
                                        </div>
                                      )}
                                    </div>
                                  </div>
                                  ...
                                  <div className="space-y-2">
                                    ...
                                    <Input
                                      ...
                                      onChange={(e) => {
                                        ...
                                        const clampedValue = Math.max(
                                          0,
                                          Math.min(RS485.SLAVE_MAX_INDOORS, value)
                                        );
                                        handleSlaveConfigChange(
                                          configIndex,
                                          slaveIndex,
                                            ...
                                            <div className="flex items-center gap-1">
                                              ...
                                              {hasUnmappedAddress(config.slave_cfg[slaveIndex]?.indoor_group?.[indoorIndex] || 0) && (
                                                <Button
                                                  ...
                                                  size="icon"
                                                  className="h-8 w-8"
                                                  onClick={() => handleAddMissingAddress(config.slave_cfg[slaveIndex]?.indoor_group?.[indoorIndex] || 0)}
                                                  title={`Add aircon address ${config.slave_cfg[slaveIndex]?.indoor_group?.[indoorIndex] || 0} to database`}
                                                >
                                              ...
                                              {!hasUnmappedAddress(config.slave_cfg[slaveIndex]?.indoor_group?.[indoorIndex] || 0) ? (
                                                ...
                                                <div className="flex-1 px-2 py-1 border rounded-md bg-muted text-muted-foreground text-xs">
                                                  Address {config.slave_cfg[slaveIndex]?.indoor_group?.[indoorIndex] || 0} (Not in database)
                                                </div>
                                              )}
                                            </div>
                                          </div>
