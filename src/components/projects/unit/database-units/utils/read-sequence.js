 ...
 */
export const readSequenceConfigurations = async (
  networkUnit,
  projectId,
  ...
  try {
    ...
    if (result?.sequences && result.sequences.length > 0) {
      console.log(
        `Found ${result.sequences.length} sequences on network unit`
      );
      ...
      for (const networkSequence of result.sequences) {
        try {
          ...
          if (
            !networkSequence.multiSceneAddresses ||
            networkSequence.multiSceneAddresses.length === 0
          ) {
            console.log(
              `Skipping sequence ${networkSequence.index}: no multi-scenes`
            );
            continue;
          }
          ...
          for (
            let i = 0;
            i < networkSequence.multiSceneAddresses.length;
            i++
          ) {
            ...
            if (multiSceneId) {
              try {
                ...
                console.error(
                  `Failed to add multi-scene ${multiSceneId} to sequence ${createdSequence.id}:`,
                  error
                );
              ...
              console.warn(
                `Multi-scene with address ${multiSceneAddress} not found in created multi-scenes`
              );
            }
          }
          ...
          console.error(
            `Failed to process sequence ${networkSequence.index}:`,
            error
          );
