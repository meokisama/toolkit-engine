  ...
  const handleInputChange = (field, value) => {
    setFormData((prev) => {
      ...
      if (field === "type" && value) {
        ...
        newData.rs485_config = Array.from({ length: 2 }, () => createDefaultRS485Config());
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (unitSupportsRS485() && !submitData.rs485_config) {
        ...
        submitData.rs485_config = Array.from({ length: 2 }, () => createDefaultRS485Config());
      }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>{mode === "edit" ? "Update the unit details below." : "Add a new unit to your project."}</DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit}>
          <div className="grid gap-6 py-4">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <Input
                      ...
                      className={`w-15 text-center [&::-webkit-inner-spin-button]:appearance-none ${errors.id_can_last_part ? "border-red-500" : ""}`}
                    />
                  </div>
                  {errors.id_can_last_part && <p className="text-sm text-red-500 mt-1">{errors.id_can_last_part}</p>}
                </div>
              </div>
            </div>
                  ...
                  <SelectContent>
                    {UNIT_MODES.map((mode) => {
                      ...
                      const isEnabled =
                        ...
                        (mode === "Stand-Alone" && unitConstraints.modes.standAlone);

                      return (
                        <SelectItem key={mode} value={mode} disabled={!isEnabled} className={!isEnabled ? "opacity-50 cursor-not-allowed" : ""}>
                          {mode}
                        </SelectItem>
                      );
                    })}
            ...
            <div className="flex gap-4">
              ...
              <div className="flex items-center space-x-2 w-1/2">
                <Checkbox
                  ...
                  disabled={
                    ...
                    (unitConstraints && !unitConstraints.checkboxes.canLoad) || (modeConstraints && !modeConstraints.canLoad.enabled)
                  }
              ...
              <div className="flex items-center space-x-2 w-1/2">
                ...
                <Label
                  ...
                  className={`text-sm font-normal ${unitConstraints && !unitConstraints.checkboxes.recovery ? "opacity-50" : ""}`}
                >
                  Line Cut Recovery
                </Label>
              </div>
            ...
            {unitSupportsRS485() && (
              <div className="flex flex-col gap-2">
                ...
                <Button type="button" variant="outline" onClick={() => setRS485DialogOpen(true)} className="flex items-center gap-2">
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={loading || !formData.type.trim() || errors.type || errors.ip_address || errors.id_can_last_part}>
      ...
      <RS485ConfigDialog open={rs485DialogOpen} onOpenChange={setRS485DialogOpen} config={formData.rs485_config} onSave={handleRS485ConfigSave} />
    </Dialog>
  );
}
