...
export function DeleteSceneDialog({ open, onOpenChange, unit }) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Delete scene(s) from unit {unit?.ip_address} (CAN ID: {unit?.id_can}
            )
          </DialogDescription>
        </DialogHeader>
          ...
          {deleteMode === "specific" && (
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                Enter scene indexes separated by commas (0-99). Example: 1, 5,
                10, 15
              </p>
            </div>
          )}
          ...
          {deleteMode === "all" && (
            <div className="p-3 bg-yellow-50 border border-yellow-200 rounded-md">
              <p className="text-sm text-yellow-800">
                ⚠️ This will delete all scenes (index 0-99) from the network
                unit.
              </p>
            </div>
          )}
