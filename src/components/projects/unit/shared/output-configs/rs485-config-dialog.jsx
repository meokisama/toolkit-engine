...
export function RS485ConfigDialog({ open, onOpenChange, config, onSave }) {
  ...
  useEffect(() => {
    if (open) {
      if (config && config.length > 0) {
        ...
        const defaultConfigs = Array.from({ length: RS485.MAX_CONFIG }, () => createDefaultRS485Config());
  ...
  const currentConfig = rs485Configs[parseInt(activeRS485Tab)] || createDefaultRS485Config();
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[900px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>Configure RS485 communication settings for the unit.</DialogDescription>
        </DialogHeader>
                ...
                <Card>
                  ...
                  <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="space-y-2">
                      <Label>Type</Label>
                      <Combobox
                        value={config.config_type === 0 ? "" : config.config_type.toString()}
                        onValueChange={(value) => handleRS485ConfigChange(configIndex, "config_type", value === "" ? 0 : parseInt(value))}
                ...
                {!isNoneTypeConfig && currentConfig.num_slave_devs > 0 && (
                  <Card>
                    ...
                    <CardContent className="space-y-4">
                      {Array.from({ length: currentConfig.num_slave_devs }, (_, slaveIndex) => (
                        <Collapsible key={slaveIndex} open={openSlaves[slaveIndex] || false} onOpenChange={() => toggleSlave(slaveIndex)}>
                          ...
                          <CollapsibleContent className="p-4 bg-muted/50 rounded-b-lg shadow-sm">
                            <div className="space-y-4">
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div className="space-y-2">
                                  ...
                                  <Input
                                    ...
                                    min="1"
                                    max="255"
                                    value={currentConfig.slave_cfg[slaveIndex]?.slave_id || 1}
                                    onChange={(e) => {
                                      const value = parseInt(e.target.value) || 1;
                                      const clampedValue = Math.max(1, Math.min(255, value));
                                ...
                                <div className="space-y-2">
                                  ...
                                  <Combobox
                                    value={
                                      currentConfig.slave_cfg[slaveIndex]?.slave_group === 0
                                        ? ""
                                        : (currentConfig.slave_cfg[slaveIndex]?.slave_group || 0).toString()
                                    }
                                    onValueChange={(value) =>
                                      handleSlaveConfigChange(configIndex, slaveIndex, "slave_group", value === "" ? 0 : parseInt(value) || 0)
                                    }
                                ...
                                <div className="space-y-2">
                                  ...
                                  <Input
                                    ...
                                    value={currentConfig.slave_cfg[slaveIndex]?.num_indoors || 0}
                                    onChange={(e) => {
                                      const value = parseInt(e.target.value) || 0;
                              ...
                              {/* Indoor Groups */}
                              {currentConfig.slave_cfg[slaveIndex]?.num_indoors > 0 && (
                                <div className="space-y-4">
                                  <Label className="text-base font-semibold">Indoor Groups</Label>
                                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                    {Array.from(
                                      {
                                        length: currentConfig.slave_cfg[slaveIndex].num_indoors,
                                      },
                                      (_, indoorIndex) => (
                                        <div key={indoorIndex} className="space-y-2">
                                          <Label>Indoor #{indoorIndex + 1}</Label>
                                          <Combobox
                                            value={
                                              currentConfig.slave_cfg[slaveIndex]?.indoor_group[indoorIndex] === 0
                                                ? ""
                                                : (currentConfig.slave_cfg[slaveIndex]?.indoor_group[indoorIndex] || 0).toString()
                                            }
                                            onValueChange={(value) =>
                                              handleIndoorGroupChange(configIndex, slaveIndex, indoorIndex, value === "" ? 0 : parseInt(value) || 0)
                                            }
                                            options={airconOptions}
                                            placeholder="Select aircon"
                                          />
