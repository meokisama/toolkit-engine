...
const SceneItem = memo(({ item, index, getObjectTypeInfo, getFormattedValue }) => {
  const typeInfo = getObjectTypeInfo(item.objectValue);
  const formattedValue = getFormattedValue(item.objectValue, item.itemValue);

  return (
    <div key={index} className="p-3 rounded-lg border bg-muted/20 transition-all hover:shadow-sm">
      <div className="flex justify-between items-center">
        ...
        <span className="text-xs font-semibold px-2 py-0.5 rounded bg-background border">{formattedValue}</span>
      </div>
    </div>
  );
});
              ...
              <PopoverContent className="w-108" align="end">
                <div className="space-y-3">
                  <div className="flex justify-between">
                    <h4 className="font-medium text-sm">{formatSceneName ? formatSceneName(scene) : scene.name || "No name"}</h4>
                    <div className="text-xs text-muted-foreground">
                      <strong>Group:</strong> {scene.address} | <strong>Total Items:</strong> {scene.itemCount}
                    </div>
                  </div>
                  ...
                  {loadingSceneDetails[scene.index] ? (
                    ...
                    <div className="space-y-2">
                      <p className="text-xs text-muted-foreground">No items found for this scene.</p>
                    </div>
                  )}
                </div>
              </PopoverContent>
...
export function TriggerSceneDialog({ open, onOpenChange, unit }) {
  ...
  const getDatabaseSceneName = useCallback(
    (address) => {
      if (!selectedProject || !projectItems.scene) return null;

      const databaseScene = projectItems.scene.find((scene) => parseInt(scene.address) === parseInt(address));
  ...
  const formatSceneName = useCallback(
    (networkScene) => {
  ...
  const handleLoadSceneInfo = useCallback(async () => {
    ...
    try {
      ...
      const result = await window.electronAPI.sceneController.getSceneInformation({
        unitIp: unit.ip_address,
        canId: unit.id_can,
        sceneIndex: protocolIndex,
      });
  ...
  const handleTriggerSceneFromCard = useCallback(
    async (sceneIndex, sceneAddress) => {
      ...
      try {
        ...
        await window.electronAPI.sceneController.triggerScene({
          unitIp: unit.ip_address,
          canId: unit.id_can,
          sceneIndex,
          sceneAddress,
  ...
  const handleConfirmDeleteScene = useCallback(async () => {
    ...
    try {
      ...
      await window.electronAPI.sceneController.deleteScene(unit.ip_address, unit.id_can, state.deleteConfirmDialog.sceneIndex);

      toast.success(`Scene ${state.deleteConfirmDialog.sceneIndex} deleted successfully`);
    ...
    async (sceneIndex) => {
      ...
      try {
        ...
        if (result && result.items) {
          ...
          setState((prevState) => ({
            ...prevState,
            scenes: prevState.scenes.map((scene) => (scene.index === sceneIndex ? { ...scene, items: result.items } : scene)),
          }));
  ...
  const handleKeyPress = useCallback(
    (e) => {
      if (e.key === "Enter" && !loadingState.loading && !loadingState.loadingInfo) {
        handleLoadSceneInfo();
      }
    },
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[900px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Load information and trigger scene on unit {unit?.ip_address} (CAN ID: {unit?.id_can}).
          </DialogDescription>
        </DialogHeader>
            ...
            <div className="flex justify-between items-center gap-4">
              <div className="flex items-center gap-2">
                <Input
                  ...
                  disabled={loadingState.loading || loadingState.loadingInfo || loadingState.loadingAllScenes}
                  autoFocus
                  className="max-w-[150px]"
                />
                <Button
                  onClick={handleLoadSceneInfo}
                  disabled={loadingState.loadingInfo || !state.sceneIndex || loadingState.loadingAllScenes}
              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  size="lg"
                  disabled={loadingState.loadingAllScenes || loadingState.loadingInfo}
                  className="flex items-center gap-2"
                >
                  <List className="h-4 w-4" />
                  {loadingState.loadingAllScenes ? "Loading..." : "Load All Scenes"}
                </Button>
              </div>
            </div>
          </div>
              ...
              {state.showScenes && state.scenes.length > 0 ? (
                ...
                <div className="flex items-center justify-center h-full mt-10">
                  <div className="text-center text-muted-foreground">
                    ...
                    <p className="text-sm">Use "Load Scene" or "Load All Scenes" to display scene information</p>
                  </div>
                </div>
              )}
            </ScrollArea>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to delete "{state.deleteConfirmDialog.sceneName}" from unit {unit?.ip_address}?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            >
              {loadingState.loading && <Loader2 className="h-4 w-4 mr-2 animate-spin" />}
