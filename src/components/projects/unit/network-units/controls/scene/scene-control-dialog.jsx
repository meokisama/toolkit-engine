...
export function TriggerSceneDialog({ open, onOpenChange, unit }) {
  ...
  const getDatabaseSceneName = useCallback(
    (address) => {
      ...
      const databaseScene = projectItems.scene.find(
        (scene) => parseInt(scene.address) === parseInt(address)
      );
  ...
  const handleLoadSceneInfo = useCallback(async () => {
    ...
    try {
      ...
      const result =
        await window.electronAPI.sceneController.getSceneInformation({
          unitIp: unit.ip_address,
          canId: unit.id_can,
          sceneIndex: protocolIndex,
        });
  ...
  const handleConfirmDeleteScene = useCallback(async () => {
    ...
    try {
      ...
      await window.electronAPI.sceneController.deleteScene(
        unit.ip_address,
        unit.id_can,
