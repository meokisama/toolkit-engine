...
export const AirconControlDialog = memo(function AirconControlDialog({ room, open, onOpenChange }) {
  ...
  const fetchRoomStatus = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const acStatus = await window.electronAPI.airconController.getACStatus({
  ...
  const sendPowerMode = useCallback(
    async (power) => {
      ...
      try {
        await window.electronAPI.airconController.setPowerMode({
          unitIp: room.unit.ip_address,
          canId: room.unit.id_can,
          group: acGroup,
          power,
  ...
  const sendACMode = useCallback(
    async (acMode) => {
      ...
      try {
        await window.electronAPI.airconController.setOperateMode({
          unitIp: room.unit.ip_address,
          canId: room.unit.id_can,
          group: acGroup,
          mode: acMode,
  ...
  const sendFanMode = useCallback(
    async (fanMode) => {
      ...
      try {
        await window.electronAPI.airconController.setFanMode({
          unitIp: room.unit.ip_address,
          canId: room.unit.id_can,
          group: acGroup,
          fanSpeed: fanMode,
  ...
  const sendTemperature = useCallback(
    async (temperature) => {
      ...
      try {
        await window.electronAPI.airconController.setSettingRoomTemp({
          unitIp: room.unit.ip_address,
          canId: room.unit.id_can,
          group: acGroup,
          temperature,
