...
import { RefreshCw, Monitor, Edit3, CalendarIcon, Clock, CheckCircle, XCircle } from "lucide-react";
    ...
    try {
      for (let i = 0; i < unitsToSync.length; i++) {
        ...
        try {
          ...
          await window.electronAPI.deviceController.syncClock({
            unitIp: unit.ip_address,
            canId: unit.id_can,
            clockData,
          });
        ...
        <div className="grid gap-6 py-4">
          {!showResults ? (
            <>
              ...
              {/* Sync Button */}
              <Button onClick={handleBulkSync} disabled={loading || selectedCount === 0} className="w-full flex items-center gap-2">
                    ...
                    {syncResults.map((result, index) => (
                      <div key={index} className="flex items-center space-x-3 p-2 rounded border">
                        ...
                        <div className="flex-1">
                          ...
                          <div className={`text-xs ${result.status === "success" ? "text-green-600" : "text-red-600"}`}>{result.message}</div>
                        </div>
                      </div>
                    ))}
                  </div>
