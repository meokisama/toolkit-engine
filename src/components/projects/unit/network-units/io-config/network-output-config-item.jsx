...
const NetworkOutputConfigItem = memo(
  ({
    ...
    const hasChanged = useMemo(() => {
      ...
      if (config.type !== "ac") {
        if (config.lightingAddress !== originalConfig.lightingAddress)
          return true;
        ...
        if (config.scheduleOnMinute !== originalConfig.scheduleOnMinute)
          return true;
        if (config.scheduleOffHour !== originalConfig.scheduleOffHour)
          return true;
        if (config.scheduleOffMinute !== originalConfig.scheduleOffMinute)
          return true;
      }
      ...
      if (config.type === "ac") {
        ...
        if (config.acHighFCU_Group !== originalConfig.acHighFCU_Group)
          return true;
        if (config.acFanAnalogGroup !== originalConfig.acFanAnalogGroup)
          return true;
        if (config.acAnalogCoolGroup !== originalConfig.acAnalogCoolGroup)
          return true;
        if (config.acAnalogHeatGroup !== originalConfig.acAnalogHeatGroup)
          return true;
        if (
          config.acValveCoolOpenGroup !== originalConfig.acValveCoolOpenGroup
        )
          return true;
        if (
          config.acValveCoolCloseGroup !== originalConfig.acValveCoolCloseGroup
        )
          return true;
        if (
          config.acValveHeatOpenGroup !== originalConfig.acValveHeatOpenGroup
        )
          return true;
        if (
          config.acValveHeatCloseGroup !== originalConfig.acValveHeatCloseGroup
        )
          return true;
        if (config.acWindowBypass !== originalConfig.acWindowBypass) return true;
        if (config.acSetPointOffset !== originalConfig.acSetPointOffset)
          return true;
        if (config.acUnoccupyPower !== originalConfig.acUnoccupyPower)
          return true;
        ...
        if (config.acUnoccupyFanSpeed !== originalConfig.acUnoccupyFanSpeed)
          return true;
        if (config.acOccupyFanSpeed !== originalConfig.acOccupyFanSpeed)
          return true;
        if (config.acStandbyFanSpeed !== originalConfig.acStandbyFanSpeed)
          return true;
        if (
          config.acUnoccupyCoolSetPoint !== originalConfig.acUnoccupyCoolSetPoint
        )
          return true;
        if (config.acOccupyCoolSetPoint !== originalConfig.acOccupyCoolSetPoint)
          return true;
        if (
          config.acStandbyCoolSetPoint !== originalConfig.acStandbyCoolSetPoint
        )
          return true;
        if (
          config.acUnoccupyHeatSetPoint !== originalConfig.acUnoccupyHeatSetPoint
        )
          return true;
        if (config.acOccupyHeatSetPoint !== originalConfig.acOccupyHeatSetPoint)
          return true;
        if (
          config.acStandbyHeatSetPoint !== originalConfig.acStandbyHeatSetPoint
        )
          return true;
      }
      ...
      >
        ...
        <div className="flex items-center gap-1">
          ...
          <Button
            ...
            title={`${autoMappedDeviceId ? "Edit" : "Create"} ${
              isAircon ? "aircon" : "lighting"
            }`}
          >
