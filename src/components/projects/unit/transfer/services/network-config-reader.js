  ...
  try {
    ...
    if (selectedProject && projectItems && createItem && createdItemsCache) {
      try {
        ...
        log.info("I/O configurations read successfully");
      } catch (error) {
        log.error("Failed to read I/O configurations:", error);
        // Continue without I/O configs
      }
    } else {
      log.info("Skipping I/O configuration read (missing project context)");
    }

    return unitWithConfigs;
  } catch (error) {
    log.error(`Failed to read basic configurations from network unit ${networkUnit.ip_address}:`, error);
