...
export function SendDmxDialog({ open, onOpenChange, items = [] }) {
  ...
  const handleSendSingleDmx = async (dmx, dmxData, selectedUnits) => {
    ...
    for (const unit of selectedUnits) {
      try {
        console.log("Sending DMX config to unit:", {
          unitIp: unit.ip_address,
          canId: unit.id_can,
          dmxAddress: dmxData.address,
          dmxName: dmxData.name,
        });
  ...
  const handleSendBulkDmx = async (dmxConfigs, selectedUnits, onProgress) => {
    ...
    for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
      ...
      for (const unit of selectedUnits) {
        ...
        try {
          console.log("Sending DMX batch to unit:", {
            unitIp: unit.ip_address,
            canId: unit.id_can,
            batchSize: validDmxConfigs.length,
            devices: validDmxConfigs.map((d) => ({ name: d.name, address: d.address })),
          });
