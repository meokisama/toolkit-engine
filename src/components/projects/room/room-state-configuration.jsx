...
import { Lightbulb, CircleCheck, Settings2, Fan, ThermometerSnowflake, ThermometerSun } from "lucide-react";
import * as CheckboxPrimitive from "@radix-ui/react-checkbox";
import { Card, CardContent } from "@/components/ui/card";

export function RoomStateConfiguration({ state, config, updateConfig, availableScenes = [] }) {
  const { airconActive = false, airconMode = 0, airconFanSpeed = 0, airconCoolSetpoint = 24, airconHeatSetpoint = 20, scenesList = [] } = config;
  ...
  const handleSceneToggle = (sceneAddress, checked) => {
    const newScenesList = checked ? [...scenesList, parseInt(sceneAddress)] : scenesList.filter((addr) => addr !== parseInt(sceneAddress));
    ...
    <Card>
      <CardContent className="space-y-6">
        ...
        <div className="flex items-center gap-2">
          <Checkbox id={`${state}-aircon-active`} checked={airconActive} onCheckedChange={(checked) => handleUpdate("airconActive", checked)} />
        ...
        <div className="grid grid-cols-4 gap-2">
          ...
          <div className="space-y-2">
            ...
            <Select value={airconMode.toString()} onValueChange={(value) => handleUpdate("airconMode", parseInt(value))}>
          ...
          <div className="space-y-2">
            ...
            <Select value={airconFanSpeed.toString()} onValueChange={(value) => handleUpdate("airconFanSpeed", parseInt(value))}>
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground mb-2 -mt-1">Select the scenes to run in this state</p>
          <ScrollArea className="h-48 rounded-md">
            {availableScenes.length === 0 ? (
              <div className="text-center text-muted-foreground py-8">
                <p>No scenes available.</p>
                <p className="text-sm">Create scenes first to add them to room states.</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-2 p-1">
                {availableScenes.map((scene) => (
                  <CheckboxPrimitive.Root
                    ...
                    onCheckedChange={(checked) => handleSceneToggle(scene.address, checked)}
                  ...
                  >
                    ...
                    <div className="space-y-0.5 flex-1 min-w-0">
                      <span className="font-medium tracking-tight text-sm block truncate">{scene.name}</span>
