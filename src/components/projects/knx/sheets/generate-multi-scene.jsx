...
export function GenerateFromMultiSceneSheet({ open, onOpenChange }) {
  const { projectItems, createItem } = useProjectDetail();
  const [loading, setLoading] = useState(false);
  const [multiSceneItems, setMultiSceneItems] = useState([]);
  ...
  useEffect(() => {
    if (open && projectItems.multi_scenes) {
      const items = projectItems.multi_scenes || [];
      setMultiSceneItems(items);

      // Initialize KNX data for each multi scene item
      ...
      items.forEach((item) => {
        initialKnxData[item.id] = {
          factor: 1,
          feedback: 0,
          knx_switch_group: "", // Single KNX address for multi scene trigger
          knx_status_group: "",
        };
      });
  ...
  }, [open, projectItems.multi_scenes]);
  ...
  const handleSelectAll = useCallback(
    (checked) => {
      if (checked) {
        setSelectedItems(new Set(multiSceneItems.map((item) => item.id)));
      } else {
        setSelectedItems(new Set());
      }
    },
  ...
  const handleCreate = useCallback(async () => {
    ...
    if (selectedItemsList.length === 0) {
      toast.error("Please select at least one multi scene item");
      return;
    }
    ...
    try {
      ...
      for (const itemId of selectedItemsList) {
        const multiSceneItem = multiSceneItems.find((item) => item.id === itemId);
        const knxItemData = knxData[itemId];

        if (!multiSceneItem || !knxItemData) continue;

        const itemData = {
          name: multiSceneItem.name || `Multi Scene ${multiSceneItem.address}`,
          address: availableAddresses[addressIndex],
          type: 5, // Fixed type for multi scene (KNX_OUTPUT_MULTI_SCENE)
          factor: parseInt(knxItemData.factor),
          feedback: parseInt(knxItemData.feedback),
          rcu_group_id: multiSceneItem.id, // Link to multi scene item
          rcu_group_type: "multi_scenes", // Specify the RCU group type
          knx_switch_group: knxItemData.knx_switch_group,
          knx_dimming_group: "", // Not used for multi scene
          knx_value_group: "", // Not used for multi scene
          knx_status_group: knxItemData.knx_status_group,
          description: `Generated from multi scene: ${multiSceneItem.name || multiSceneItem.address}`,
        };
      ...
      results.forEach((result, index) => {
        if (result.status === "fulfilled") {
          ...
          errorCount++;
          log.error(`Failed to create KNX item for multi scene ${selectedItemsList[index]}:`, result.reason);
        }
      });
  ...
  }, [selectedItems, multiSceneItems, knxData, findAvailableKnxAddresses, createItem, onOpenChange]);

  const selectedCount = selectedItems.size;
  const totalCount = multiSceneItems.length;
      ...
      <SheetContent side="left" className="sm:max-w-[900px] w-full max-w-[90vw] p-4 pb-0">
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-2 p-3 bg-muted/50 rounded-lg font-medium text-sm">
            ...
            <div className="w-[24%]">Multi Scene</div>
