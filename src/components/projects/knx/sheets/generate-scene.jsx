...
export function GenerateFromSceneSheet({ open, onOpenChange }) {
    ...
    try {
      ...
      results.forEach((result, index) => {
        if (result.status === "fulfilled") {
          ...
          errorCount++;
          console.error(`Failed to create KNX item for scene ${selectedItemsList[index]}:`, result.reason);
        }
      });
  ...
  }, [selectedItems, sceneItems, knxData, findAvailableKnxAddresses, createItem, onOpenChange]);
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="left" className="sm:max-w-[900px] w-full max-w-[90vw] p-4 pb-0">
        <SheetHeader>
          ...
          <SheetDescription>Select scene items to generate corresponding KNX devices.</SheetDescription>
        </SheetHeader>
                    ...
                    <div className="w-[24%]">
                      <div className="flex items-center gap-2">
                        <span className="truncate">
                          {item.name || `Scene ${item.address}`} <span className="font-light">({item.address})</span>
                        </span>
                      </div>
                    </div>
