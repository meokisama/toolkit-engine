...
namespace RLC
{
    public partial class Input_RLC_new : Form
    {
        /****************************Output*************************/

        const int SLRelay_AO = 32;
        const int SLDimmer = 6;
            ...
            for (int i = 0; i < 1; i++)
            {
                ...
                for (int j = 0; j < SLInput_RLC; j++)
                {
                    ...
                    if (XForm.confignetwork == true)
                    {
                        ...
                        //Input_On[dem].Location = new Point(lb.Size.Width + 20, loc_y - 5);
                        Input_On[dem].Location = new Point(lb.Size.Width + 42, loc_y - 5);  //edited by Hoai An
            ...
            while ((input = rd.ReadLine()) != null)
            {
                ...
                else
                {
                    ...
                    if (dem < SLInput_RLC)
                    {
                        ...
                        int tmp, numInt, numDecimal;
                        numInt = (dem + 1) / 3;
                        numDecimal = (dem + 1) % 3;
                        tmp = board_att.Get_Function_Index(function_list, IO[dem].Function);
                        if (tmp < cbx_Function[dem].Items.Count)
                            cbx_Function[dem].SelectedIndex = board_att.Get_Function_Index(function_list, IO[dem].Function);
                        else
                            MessageBox.Show("Error: Type of Input " + (numInt).ToString() + "." + (numDecimal).ToString() + " is unknown. Please don't save config and use RLC Toolkit with higher verion");
                    }
                    else
                    {
                        op++;
        ...
        private void Input_RLC_Load(object sender, EventArgs e)
        {
...
#else
            for (int i = 12 * 3; i < 16 * 3; i++)
            {
                cbx_Group[i].Enabled = false;
                cbx_Function[i].Enabled = false;
                Add_Group[i].Enabled = false;
                Edit_Group[i].Enabled = false;
                Config_IO[i].Enabled = false;
            }
#endif
            ...
            cbx_Function[6].Items.Add("SW_DND");
            ...
            cbx_Function[7].Items.Add("SW_MUR");
        ...
        private void Config_Click(object sender, EventArgs e)
        {
            ...
            ConfigIO_RLC_new Config = new ConfigIO_RLC_new();            
        ...
        private void AddComponentOutput()
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                for (int j = 0; j < inputh; j++)
                {
                    ...
                    {
                        if (inputl <= 8)
                            lb.Text = "Relay 1." + inputl.ToString();
                        else if (inputl <= 16)
                        {
                            ip = inputl - 8;
                            lb.Text = "Relay 2." + ip.ToString();
                        }
                        else if (inputl <= 24)
                        {
                            ip = inputl - 16;
                            lb.Text = "Relay 3." + ip.ToString();
                        }
                        else
                        {
                            ip = inputl - 24;
                            lb.Text = "Relay 4." + ip.ToString();
                        }
                    }
                    else
                    {
                    ...
                    if (XForm.confignetwork == true)
                    {
                        if (dem < SL_Lighting_Out)
                        {
                            ...
                            //Relay_On[dem].Location = new Point(loc_x + 77, loc_y - 5);    //edit by Hoai An
